...
namespace FFXIV_TexTools2.Helpers
{
    ...
    public static class FFCRC
    {
        ...
        static uint[] crc_table_1 = new uint[]{
        ...
        static uint[] crc_table_2 = new uint[]{
        ...
        static uint[] crc_table_3 = new uint[]{
        ...
        static uint[] crc_table_4 = new uint[]{
            ...
            using (MemoryStream stream = new MemoryStream(ecxBytes))
            {
                using (BinaryReader reader = new BinaryReader(stream))
                {
                    ...
                    for (int i = 0; i < cbRunningLength / 4; ++i)
                    {
                        ...
                        dwCRC = unchecked((int)(crc_table_4[dwCRC & 0x000000FF] ^ crc_table_3[(dwCRC >> 8) & 0x000000FF] ^ crc_table_2[(dwCRC >> 16) & 0x000000FF] ^ crc_table_1[(dwCRC >> 24) & 0x000000FF]));
                    }
                    ...
                    for (int i = 0; i < cbEndUnalignedBytes; ++i)
                    {
                        ...
                        if (dwCRC < 0)
                        {
                            dwCRC = unchecked((int)(crc_table_1[(dwCRC ^ b) & 0x000000FF] ^ ((uint)dwCRC >> 8)));
                        }
                        else
                        {
                            dwCRC = unchecked((int)(crc_table_1[(dwCRC ^ b) & 0x000000FF] ^ (dwCRC >> 8)));
                        }
                    }
                }
            }
        ...
        {
            return ComputeCRC(Encoding.Default.GetBytes(name));
        }
    }
}